...
export const expresiones = {
  ...
  numeros: /^[0-9]{1,20}$/,
  todo: /^[a-zA-Záãäéëêíîóöúüñç0-9ñÑ)(%+@#!$^=&*_.,:"'/|;`~-]{6,3040}$/,
...
export function valid(form, target,) {
  ...
  let required = "This dato is required";
  let numinvalid =
    "You must enter a numerical value that does not exceed 20 digits";
  let nameinvalid =
    "minimum 3 and maximum 60 characters, special characters allowed ()%+-";
  ...
  if (!form.image.trim() && target === "image") {
    errors.image = required;
  } else {
    if (!expresiones.todo.test(form.image.trim()) && form.image.trim()) {
      errors.image = descriptioninvalid;
    } else errors.image = "";
  } valid2(form)
