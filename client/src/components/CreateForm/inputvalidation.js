...
export const expresiones = {
  name: /^[a-zA-ZáãäéëêíîóöÔúüñç0-9ñÑ)(%+&,#' _.-]{3,60}$/, // Letras, numeros, guion y guion_bajo
  numeros: /^[0-9.]{1,20}$/,
  numeros2: /^[0-9.]{1,3}$/,
  todo: /.{4,1500}$/,
  isLink:
...
export function valid(form, target) {
  ...
  let nameinvalid = "Minimum 3 and maximum 60 characters";
  let descriptioninvalid = "Minimum 4 and maximum 3000 characters";
  let linkInvalid = "Invalid link";
  let discountInvalid = "you must enter a value between 0 and 100";
  ...
  if (!form.discount && form.discount !== 0 && target === "discount") {
    ...
    if (
      (!expresiones.numeros.test(form.discount) ||
...
export function valid3(form) {
  if (form.image === "Upload Image" || form.image === "") {
    errors.image = "you must upload an image of the product";
  } else errors.image = "successful upload";
  return { errors, validation };
}
