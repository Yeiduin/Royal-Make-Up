...
export const NewArrivalsCard = ({ name, price, image_link, rating, description, discount }) => {
  return (
    <div className="flex-wrap">
      ...
      <div className="h-52 w-60 px-4 flex flex-col items-start justify-between">
        ...
        <div>
        {discount
      ? (<h5 className="text-secondary"><span className="line-through">${price}</span><span className="font-bold text-base"> ${discount}</span></h5>) 
      : (<h5 className="text-secondary">${price}</h5>)
      }
        </div>
        <div ><span className="text-xs material-icons text-secondary">star</span> <span>{rating}</span>
        </div>
        </div>
